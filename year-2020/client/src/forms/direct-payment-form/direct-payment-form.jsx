...
const directTypes = [
      ...
      checkLineRegex = createRegex`
        ^\d+
        \s+
        (?<irn>\d{8,})
        \s+
        (?<date>\d{1,2}\/\d{1,2}\/20[0-9]{2})
        \s+
...
export default class DirectPaymentForm extends Component {
  constructor(props) {
    ...
    this.action = '/admin/accounting/checks'
  }
      ...
      this.setState({submitting: true}, async () => {
        try {
          const form = Objected.deepClone(this.state.form),
                ...
                result = await fetch(`/admin/users/${dusId}/main_address`),
  ...
  parseChecksPaste = (ev) => {
    const rawZions = ev.target.value.replace(/\n\n/g, "\n"),
          lines = rawZions.split(/\n/).filter((v) => checkLineRegex.test(v))
    ...
    for(let i = 0; i < lines.length; i++) {
      const line = lines[i],
            parsed = line.match(checkLineRegex)
      if(parsed) {
        const { irn, date, time, routing, account, check, amount: rawAmount } = parsed.groups || {},
              date_entered = date ? this.americanToUniversal(date) : '',
              amount = currencyFormat(rawAmount || '')
        total = total + +amount
        form.payment.gateway.deposit_number = form.payment.gateway.deposit_number || irn
        form.payment.date_entered = form.payment.date_entered || date_entered
        form.payment.checks.push({
          ...
          gateway: {
            ...
            account_number: account.slice(account.length - 4),
            check_number: check,
            send_email: false,
          },
          split: [],
    ...
    form.payment.amount = currencyFormat(total)
    form.payment.gateway.deposited_items = form.payment.checks.length
                ...
                <div className="alert alert-success form-group" role="alert">
                  {
                    this.state.receipts && this.state.receipts.map((v, k) => (
                      <div className='row form-group' key={k}>
                        <div className="col">
                          <Link className='btn btn-secondary btn-block' to={`https://www.downundersports.com/payments/${v}`} target='_view_receipt'>View Receipt: {v}</Link>
                        </div>
                      </div>
                    ))
                  }
                      ...
                      fields: [
                        ...
                        ...(
                          this.allowNext() ? [
                            ...
                            {
                              field: 'TextField',
                              wrapperClass: `col-md-6 form-group ${this.state.form.payment.gateway.batch_number_validated ? 'was-validated' : ''}`,
                              label: 'Batch Number',
                              name: 'payment.gateway.batch_number',
                              placeholder: '(3)',
                              type: 'text',
                              value: this.state.form.payment.gateway.batch_number || '',
                              onChange: true,
                              autoComplete: `invalid ${autoComplete}`,
                              required: true,
                            },
                                  ...
                                  fields: [
                                    {
                                      ...
                                      fields: [
                                        ...
                                        },
                                        {
                                          ...
                                          pattern: '[0-2]?[0-9]:[0-6][0-9](:[0-6][0-9])?( [AP]M)?',
                                          inputMode: 'numeric',
                                          type: 'text',
