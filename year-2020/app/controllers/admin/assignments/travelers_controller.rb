        ...
        respond_to do |format|
          ...
          format.json do
            ...
            @assignments = base_assignments.
              order(*get_sort_params, :joined_at, :created_at, :name, :id).
          ...
          format.xlsx do
            return run_an_api_action do
              ...
              FileMailer.
                with(
                  email: value.id,
                  name: 'traveler_assignment_status',
                  mime_type: 'xlsx',
                  handler: 'axlsx',
                  extension: 'xlsx',
                  template: 'admin/assignments/travelers/index',
                  message: 'Here is your Traveler Status Worksheet',
                  subject: 'Traveler Assignments Status Worksheet'
                ).
                send_file.
            ...
            CSV.parse(uploaded, headers: true, encoding: 'utf-8').each do |row|
              begin
                ...
                if row[:staff_dus_id].present?
                  ...
                  raise "User not traveling: #{user.dus_id}" unless user.traveler

                  raise "User already assigned: #{user.dus_id}" if
                    user.staff_assignments.travelers.exists?

                  user.staff_assignments.responds.update(completed: true, completed_at: user.traveler.joined_at)
