  ...
  class MessagesController < Admin::ApplicationController
    ...
    def new
      ...
      return render json: {
        ...
        type: model_type,
        category: model.default_category,
        reason: model.default_reason,
        message: nil,
        reviewed: false,
    ...
    def update
      save_message authorize(@found_user.__send__(model_type.pluralize).find_by(id: params[:id])), 'update!'.to_sym
    end
    ...
    private
      ...
      def save_message(message, method = 'create!'.to_sym)
        ...
        begin
          params[:message][:staff_id] = current_user.category_id unless current_user.staff&.admin? && message.respond_to?(:staff_id) && message.staff_id.present?
