    ...
    class ChecksController < ::Admin::PaymentsController
      ...
      def create
        ...
        whitelist[:checks].each_with_index do |check, i|
          ...
          check[:transaction_id] = "#{gateway[:transaction_type]}#{gateway[:deposit_number].to_s.rjust(6, '0')}-#{gateway[:account_number].to_s.rjust(4, '0')[-4..-1]}-#{gateway[:check_number].to_s.rjust(6, '0')}".upcase
          payments << [
            u,
            check,
        ...
        return render json: {
          payments: payments.map do |user, check, attrs|
            if existing = Payment.find_by(transaction_id: check[:transaction_id])
              {
                check: check,
                json: {
