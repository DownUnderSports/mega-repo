  ...
  module Traveling
    module Flights
      class SchedulesController < ::Admin::ApplicationController
        ...
        def new
          @schedule = Flight::Schedule.new
          return show
        end

        def create
          run_a_schedule_action do
            @schedule = Flight::Schedule.create parse_html_params

            unless @schedule.persisted?
              raise @schedule.errors.full_messages.join("\n")
            end
        ...
        def update
          run_a_schedule_action do
            unless (@schedule = Flight::Schedule[params[:id]]).update(parse_html_params)
              raise @schedule.errors.full_messages.join("\n")
            end
        ...
        private
          ...
          def parse_html_params
            ...
            if whitelisted[:legs_attributes].present?
              time_reg = /202[0-9]-(?:0[1-9]|1[0-2])-(?:(?:0[1-9]|1[0-9]|2[0-9])|3(?:0|1))\s(?:0[1-9]|1[0-2]):[0-5][0-9]\s(?:A|P)M/
            ...
            [
              ...
              whitelisted[k] = whitelisted[k].presence&.to_i || @schedule&.__send__(k).to_i
            end
