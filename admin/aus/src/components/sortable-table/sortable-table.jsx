...
export default class SortableTable extends Component {
  ...
  pagination() {
    return (
      <div className="btn-toolbar form-group justify-content-between col d-print-none" role="toolbar" aria-label="Table Pagination">
        {
          (
            (this.state.pages > 5)
            &&
        ...
        <div className="btn-group d-print-none" role="group" aria-label="Navigate List">
          {
        ...
        </div>
        {
          (
            (this.state.pages > 5)
            &&
            <div className="btn-group d-print-none" role="group" aria-label="End Of List">
              {
                Array(2).fill().map((_, i) => (
  ...
  printPage = () => {
    if(this._isPrinting) return false;
    ...
    new Promise(r => this.setState({ printing: true }, r))
    .then(this.waitForReady)
    .then(window.print)
  }

  waitForReady = () => new Promise(r => {
    setTimeout(() => {
      if(document.getElementById('table_is_ready_to_print')) return r()
      else return this.waitForReady().then(r)
    }, 250)
  })
  ...
  render() {
    ...
    return (<section className='row'>
      <div className="col-md-6 d-print-none">
        {
          !!this.props.printable && !this.state.printing && (
            <div className="row">
              <div className="col">
                <button type="button" className="btn btn-info brn-block" onClick={this.printPage}>
                  Print Full Table
                </button>
              </div>
            </div>
        ...
        { !!this.state.printing && <div id="table_is_ready_to_print"></div> }
        { this.props.children }
      </div>
      <div className="col-md-6 d-print-none float-right form-group">
      ...
      <div className="col-12">
        <div className="table-nowrap">
          <table className='table table-bordered table-striped'>
              ...
              <tr className='clickable'>
                {
                  this.state.headers.map((h, i) => {
                    const onClick = (e) => this.sortBy(i, e.shiftKey || e.ctrlKey || e.metaKey ),
                          key = this.state.headerAliases[h] || h.split("_").join(' '),
                          sort = this.checkSort(h);

                      return (
                        <th onClick={onClick} key={key}>
            ...
            <tbody>
              {
                (this.state.printing ? this.state.fullData : this.state.data).map((datum, i) => (<tr key={`row_${i}`}>
                  {
                    this.state.headers.map((header, j) => (<td key={(i+1)*j}>
                      {this.props.renderer ? this.props.renderer(datum, header) : datum[header]}
                    </td>))
                  }
                </tr>))
              }
