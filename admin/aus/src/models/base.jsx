...
export default class BaseModel {
  ...
  transformData = (record, toServer = false) => {
    const modelSchema = (currentSchema.stores[this.storeName] || {}).transformKeys
    ...
    for(let k in modelSchema) {
      transformed[toServer ? k : modelSchema[k]] = record[k] || record[modelSchema[k]]
    }
  ...
  getServerData = async () => {
    try {
      const response = await fetch(
        `/aus/get_model_data/${this.modelName}.json?last_updated=${this.lastUpdated || ''}`,
        {
          headers: {
  ...
  parseServerData = async(response) => {
    ...
    return await this.handleResult(result, true)
  }

  saveRecord = async (record) => {
    if(this._loading) await this._loading

    return await this.handleResult({ records: [ this.transformData(record) ] })
  }

  handleResult = async (result, fromServer = false) => {
    if(result.deleted) {
      await deleteFromDB({
        storeKey: this.storeName,
    ...
    if(result.records && result.records.length) {
      ...
      if(fromServer) this.offline = false
